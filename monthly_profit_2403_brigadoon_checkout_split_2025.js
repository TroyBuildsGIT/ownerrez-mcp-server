// Monthly Profit Analysis for 2403 Brigadoon (Short-term Rental) - 2025
...
// Main analysis function with proportional splitting for Brigadoon
async function monthlyProfitAnalysisBrigadoonProportionalSplit() {
  console.log('🚀 Starting Monthly Profit Analysis for 2403 Brigadoon (Proportional Split)...\n');

  try {
    // Step 1: Get the Brigadoon property
    ...
    // Find 2403 Brigadoon property (need to check the actual property ID)
    const brigadoon = allProperties.find(p =>
      p.name && p.name.toLowerCase().includes('brigadoon')
    );

    if (!brigadoon) {
      console.log('⚠️ Brigadoon property not found. Available properties:');
      allProperties.forEach(p => console.log(`   - ${p.id}: ${p.name}`));
      throw new Error('2403 Brigadoon property not found in OwnerRez');
    }

    console.log(`✅ Found: ${brigadoon.name} (ID: ${brigadoon.id})`);

    // Step 2: Fetch all 2025 bookings for Brigadoon
    console.log('\n📅 Fetching 2025 bookings...');
    const fromDate = '2025-01-01';
    const toDate = '2025-12-31';

    const bookingsResponse = await orFetch(
      `/v2/bookings?property_ids=${brigadoon.id}&from=${fromDate}&to=${toDate}&limit=500&include_guest=true`
    );
    ...
    const expensesCsv = fs.readFileSync('data/raw/rocket_money/2025/2403 brigadoon expenses 2025.csv', 'utf8');
    ...
    console.log('\n' + '='.repeat(140));
    console.log('💵 MONTHLY PROFIT ANALYSIS - 2403 BRIGADOON - 2025 (PROPORTIONAL SPLIT)');
    ...
    const results = {
      ...
      methodology: {
        ...
        expensesSource: '2403 brigadoon expenses 2025.csv (if available)',
        importantNote: '✅ This provides the most accurate monthly revenue attribution for 30-day minimum stays'
      }
    };

    fs.writeFileSync('monthly_profit_2403_brigadoon_checkout_split_2025.json', JSON.stringify(results, null, 2));
    console.log('\n💾 Detailed results saved to monthly_profit_2403_brigadoon_checkout_split_2025.json');

    console.log('\n✅ Monthly Profit Analysis for 2403 Brigadoon (Proportional Split) complete!');

  } catch (error) {
    console.error('❌ Error during Brigadoon proportional split analysis:', error.message);
    console.error(error.stack);
  }
}

// Run the analysis
monthlyProfitAnalysisBrigadoonProportionalSplit();
